...
**Goal:** Build the hourly Python worker that checks all enabled booking jobs and executes due bookings via the Eversports API.

**Architecture:** Standalone Python script (`worker/worker.py`) that imports shared logic from `backend/core/` (booking, encryption). Runs as a K8s CronJob (`0 * * * *`). Reads jobs from PostgreSQL, skips already-booked ones, writes results to `booking_logs`. Errors in one job do not stop processing of others.
...
git commit -m "feat(worker): hourly booking worker with full test coverage"
```
